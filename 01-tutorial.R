...
# https://github.com/anpatton/basic-nba-tutorials/blob/main/win_probability/xgboost_parallel.R
library(tidyverse)
library(arrow)
# library(knitr)
set.seed(42)

df <- arrow::read_parquet("data/pbp.parquet") 
df
pbp2 <- arrow::read_parquet("data/pbp.parquet") %>% 
...
path <- file.path('data', 'fit.rds')
xgb_tuned %>% write_rds(path)
